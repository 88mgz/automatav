...
/** Build a strict system/user prompt that asks for JSON only in our Article schema. */
function buildPrompt(input: {
  ...
  const lines: string[] = []
  lines.push(
    `You are a content generator that outputs ONLY JSON for a vehicle article. Produce an object matching this TypeScript shape:`,
  )
  lines.push(`
{
  "title": string,
  ...
  "blocks": Array<
    ...
    | { "type": "comparisonTable", "items": any[] }
    | { "type": "specGrid", "items": any[] }
  ...
  if (input.tldr) lines.push(`TLDR: ${input.tldr}`)
  if (input.keyTakeaways?.length) lines.push(`Key Takeaways: ${input.keyTakeaways.join(" | ")}`)
  if (input.quizInstructions) lines.push(`Quiz: ${input.quizInstructions}`)
  if (input.calculatorInstructions) lines.push(`Calculator: ${input.calculatorInstructions}`)
  if (input.pullQuote) lines.push(`Pull Quote: ${input.pullQuote}`)
  if (input.pullQuoteAttribution) lines.push(`Pull Quote Attribution: ${input.pullQuoteAttribution}`)
  if (input.dropdownTitle) lines.push(`Dropdown Title: ${input.dropdownTitle}`)
  if (input.dropdownBody) lines.push(`Dropdown Body: ${input.dropdownBody}`)
  if (input.reviewsInstructions) lines.push(`Reviews: ${input.reviewsInstructions}`)
  if (input.models?.length) lines.push(`Models JSON: ${JSON.stringify(input.models)}`)

  lines.push(`
Rules:
- Output JSON ONLY. No backticks, no prose, no prefix/suffix.
- Provide a reasonable "slug" derived from the title (kebab-case).
- Ensure "toc" exists (an array of section titles).
- If you include "hero.image" as an object, include "alt" text.
- Only use these block types: intro | markdown | comparisonTable | specGrid | prosCons | gallery | faq | ctaBanner.
- If unsure, prefer "markdown" with "content".
`.trim())

  return lines.join("\n\n")
}

/** Minimal client-side normalization so parseArticle won't explode on minor shape differences. */
function normalizeForSchema(input: any) {
...
export default function GeneratePage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Parse models JSON (optional)
      let models: any[] | undefined = undefined
      if (formData.modelsJson && formData.modelsJson.trim()) {
        try {
      ...
      const prompt = buildPrompt({
        ...
        quizInstructions: formData.quizInstructions || undefined,
        calculatorInstructions: formData.calculatorInstructions || undefined,
        pullQuote: formData.pullQuote || undefined,
        pullQuoteAttribution: formData.pullQuoteAttribution || undefined,
        dropdownTitle: formData.dropdownTitle || undefined,
        dropdownBody: formData.dropdownBody || undefined,
        reviewsInstructions: formData.reviewsInstructions || undefined,
        models,
      })
      ...
      if (!data?.article) {
        throw new Error("Missing 'article' in response")
      }

      const validatedArticle = parseArticle(normalizeForSchema(data.article))
